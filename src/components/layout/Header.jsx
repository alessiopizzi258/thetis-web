...
const Header = () => {
  ...
  const navLinks = [
    ...
    { name: 'La Ginestra', path: '/articoli' },
  ...
  const useWhiteText = (isDarkPage && !isScrolled) || mobileMenuOpen;
  ...
  return (
    <header className={`fixed w-full z-[100] transition-all duration-700 ease-in-out ${
      ...
      <nav className="max-w-7xl mx-auto px-6 flex justify-between items-center">
        ...
        <button onClick={() => setMobileMenuOpen(!mobileMenuOpen)} className="md:hidden z-[110] p-2">
          {mobileMenuOpen ? <X size={28} className="text-ivory" /> : <Menu className={useWhiteText ? 'text-ivory' : 'text-midnight'} size={28} />}
        </button>
      </nav>

      <AnimatePresence>
        {mobileMenuOpen && (
          <motion.div initial={{ x: "100%" }} animate={{ x: 0 }} exit={{ x: "100%" }} transition={{ duration: 0.6, ease: [0.22, 1, 0.36, 1] }} className="fixed inset-0 bg-midnight z-[105] flex flex-col items-center justify-center space-y-10">
            {navLinks.map((link) => (
              <Link key={link.path} to={link.path} onClick={() => setMobileMenuOpen(false)} className="text-ivory text-4xl font-serif italic hover:text-gold-custom transition-colors">
                {link.name}
              </Link>
            ))}
          </motion.div>
